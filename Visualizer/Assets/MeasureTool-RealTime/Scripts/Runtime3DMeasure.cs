...
public class Runtime3DMeasure : MonoBehaviour
{
    ...
    //Set size to font and text characters
    [SerializeField, Range(0.1f, 2f)] private float textCharacterSize = 0.5f; 
    [SerializeField, Range(50, 500)] private int textFontSize = 300; 
    ...
    //List of colors to line material to point mode
    private List<Color> lineColors = new List<Color> {Color.red, Color.blue, Color.yellow, Color.green, Color.magenta, Color.cyan};

    //Create a index to manage set colors
    ...
    private void DrawHitLine(GameObject pointA, GameObject pointB)
    {
        ...
        distanceText.text = $"{distance:F2} m";*/ //DEPRECIATED
        ...
        //Clone material to every material has a different color
        Material lineMaterial = new Material(lineRenderer.material);

        //Get colors from index colors
        Color lineColor = lineColors[colorIndex];

        //Set new line material w/ line color
        lineMaterial.color = lineColor;

        //Set line renderer w/ line material color
        lineRenderer.material = lineMaterial;

        //Update index with next index color
        colorIndex = (colorIndex + 1) % lineColors.Count;
        ...
        //Pos a little bit up text to better visibility
        midPoint += Vector3.up * 0.1f; 
        ...
        //Set text color equal than line color
        distanceText.color = lineColor;

        //Set text size from variables
        distanceText.characterSize = textCharacterSize;
        distanceText.fontSize = textFontSize;
    }
